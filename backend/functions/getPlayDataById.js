...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    // Query the database for play data
    const query = `
      SELECT play_data, gamepk
      FROM game_detail 
      WHERE gamepk = $1 
      AND play_data IS NOT NULL 
    `;
    ...
    if (result.rows.length === 0) {
      return {
        ...
        headers,
        body: JSON.stringify({
          error: 'Play data not found for the specified gamepk'
        })
      };
    }
